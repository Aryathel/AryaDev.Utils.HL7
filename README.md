...
Reference the serializer project (which includes the domain layer):

```xml
<ProjectReference Include="path\to\AryaDev.Utils.HL7.Serializer\AryaDev.Utils.HL7.Serializer.csproj" />
```

Build the solution:

```bash
dotnet build AryaDev.Utils.HL7.sln
```
